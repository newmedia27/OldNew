...
//        $sql = "SELECT * FROM idx_shop_products_content WHERE MATCH ('$q') LIMIT 10000 OPTION ranker = WORDCOUNT";
//        $data = \Yii::$app->sphinx->createCommand($sql)->queryAll();

$sql = new Query();
        $data = $sql->from('idx_shop_products_content')->match($q)->addOptions(['field_weights'=>['name_ru'=>10,'description'=>1],'ranker'=>'WORDCOUNT'])->limit(1000)->addOptions(['field_weights'=>['name_ru'=>10,'description'=>1],'ranker'=>WORDCOUNT])->all();
        $ids = ArrayHelper::map($data, 'id', 'id');

        $query = ShopProducts::find()->where(['id' => $ids]);
